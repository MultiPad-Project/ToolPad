...
public class MakePads {
    ...
    public class Pads {
        ...
        public void defaultRotationSetting(){
            forAllChildInstance(ForAllChild.FOR_ALL, (pad, padInfo)-> defaultRotationSetting(padInfo.row, padInfo.colum));
        }
        ...
        public void padWatermark(int row, int colum, boolean light){
            View watermark = getPadView(row, colum).findViewById(PadInfo.PadLayerType.BTN_);
            if(watermark != null) watermark.setVisibility(light ? View.VISIBLE : View.INVISIBLE);
        }

        public void clearLeds(){
            forAllChildInstance(MakePads.ForAllChild.FOR_ALL, (View pad, MakePads.ChildInfo info) -> {
                    if(info.getType() == MakePads.PadType.CHAIN ||
                            info.getType() == MakePads.PadType.PAD ||
                            info.getType() == MakePads.PadType.PAD_LOGO
                    ) setLedColor(info.getRow(), info.getColum(), 0);
                });
        }
    }
    ...
    public interface OnPadCreated {
        void padCreated(RelativeLayout pad, PadInfo padInfo);
    }
        ...
        for (byte r = 0; r < rows; r++) {
            for (byte c = 0; c < columns; c++) {
                ...
                if ((r == 0 && c == 0) || (r == rows-1 && c == 0) || (r == 9 && c == columns-1)) {
                    ...
                    if(onPadCreated != null) onPadCreated.padCreated(pad, mPads.getPadInfo(r, c));
                }
            }
        }
        return mPads;
